...
- 环绕菜单整环一次性绘制：统一毛玻璃配色、修复扇区重叠伪影、鼠标移走后 2 秒宽限关闭
- Skills 辅助窗：整合标题栏 + 始终可见关闭按钮；按 8 类分类树浏览（默认扫描扩至 64 个 skills）
- 余额角标常显（`--` 占位 / 失败红色提示）；环绕菜单打开时自动隐藏角标
- 移除 AI 自动首启；新增本地翻译 skill（`~/.codex/skills/agentfloat-skills-translator/`）

见 `versions/v1.0.6/CHANGELOG.md`。
