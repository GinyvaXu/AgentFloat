...
- 修复：环绕菜单弹出（`frameGeometry` 误判导致自动关闭 → 改用 `geometry` 锚定）
- Skills 辅助窗：中英对照 / 中文 / English 三态切换，内置 52 项翻译表
- 去除 AI 自动优化功能；SKILL.md frontmatter 块标量解析增强
- 错误日志：会话内收集，关闭程序时统一导出 `logs/reports/v{版本}_{时间戳}_errors.txt`

见 `versions/v1.0.4/CHANGELOG.md`。
